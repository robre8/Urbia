...
export default function UserMenu() {
  ...
  return (
    <div className='relative z-[9999]' ref={menuRef}>
      ...
      {isOpen && (
        <Card className='absolute right-0 mt-2 w-64 p-4 bg-white rounded-xl shadow-lg border overflow-hidden'>
          {/* Sección de usuario */}
          <div className='border-b pb-3 flex items-center gap-3'>
          <div className="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center shrink-0">
  {user?.avatar ? (
    <img src={user.avatar} alt={user.nombre || "Usuario"} className="w-full h-full object-cover" />
  ) : (
    <User className="w-6 h-6 text-gray-400" /> // Aumentamos el tamaño del icono
  )}
</div>
